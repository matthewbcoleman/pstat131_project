...
* KNN: We will fit a K-Nearest Neighbors model with optimal K selected by cross validation. 
...
```{r reg_tree, fig.cap= 'Regression Tree'}
set.seed(123)
...
The regression tree is more intricate than the classification tree. The main split is on the room type of the listing, and then both splits are made on the neighbourhood group. The last splits made are on the location of the the listing.
...
```{r var_imp, fig.cap = 'Variable Importances for RF Models'}
par(mfrow = c(2,1))
...
for(lambda in lambdas){
  boost.m.mod <- gbm(as.character(price_above) ~ latitude + longitude +
                     ...
                     n.trees = best.tree, shrinkage = lambda, distribution = 'bernoulli')
  boost.m.pred <- predict(boost.m.mod, test, n.trees = best.tree, type = 'response')
...
The default shrinkage of `r best.class.lambda` was the best shrinkage value to classify if a listing is above the median price.
...
plot(svm(price_above ~ longitude+latitude, data = train, kernal = "polynomial", cost = 10), test[,c("price_above", "longitude", "latitude")])
...
```{r}
x <- cbind(train$latitude, train$longitude)
g <- train$price_above
xnew <-cbind(test$latitude, test$longitude)
mod15 <- knn(x, xnew, g, k=2, prob=TRUE)

prob <- attr(mod15, "prob")
prob <- ifelse(mod15=="1", prob, 1-prob)
px1 <- seq(min(x[,1]), max(x[,1]), .01)
px2 <- seq(min(x[,2]), max(x[,2]), .01)

prob15 <- matrix(prob, length(px1), length(px2))
# par(mar=rep(2,4))
contour(seq(min(x[,1]), max(x[,1]), .01), seq(min(x[,2]), max(x[,2]), .01), prob15, levels=1, labels="", xlab="", ylab="", main=
        "15-nearest neighbour", axes=TRUE)
