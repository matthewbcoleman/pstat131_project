...
```{r}
set.seed(123)
loc.tree <- rpart(price_above~latitude + longitude + minimum_nights + reviews_per_month +
                       neighbourhood_group +  room_type + calculated_host_listings_count, data = train)

fancyRpartPlot(loc.tree, digits = 6)
```
...
fancyRpartPlot(tree.reg, digits = 6)
```
...
```{r, echo = FALSE}
methods <- c('Tree','Bagging','Boosting')
```
...
# tune.linear <- tune(svm, price_above ~ longitude+latitude, data = train, kernel = "linear", ranges = list(cost = seq(.05, .15, .01)))
...
```{r}
# tune.poly <- tune(svm, price_above ~ longitude+latitude, data = train, kernel = "polynomial", ranges = list(cost = c(10, 15, 20)))
best.poly <- svm(price_above ~ longitude+latitude, data = train, kernal = "polynomial", cost = 10)
...
# tune.rad <- tune(svm, price_above ~ longitude+latitude, data = train, kernel = "radial", ranges = list(cost = seq(5, 15, 1)))
