...
The following project was created with the purpose of predicting prices of Airbnb rentals in New York City using various machine learning models. The dataset was downloaded from Kaggle, the original source being from insideairbnb.com. Contained in the data are nearly 50,000 observations of Airbnb listings in the NYC area with a variety of attributes including price, room type, lattitude and longitude, and information on the host of said listing. Our models varied in complexity and accuracy, and no model was 100% correct, though many ended up useful. As the old adage goes, there is no free lunch, and in the end, we concluded a logistic regression was the best model as it served as the ideal combination of interpretability and accuracy.  
...
The ROC curve above shows how the true positive rate and false positive rate change based on the threshold we choose. The AUC for our logistic classifier is `r auc`, demonstrating our classifier is very good. Now, we can find the best threshold value for out logistic model:
...
The linear discriminant analysis misclassification rate is `r lda.miss` and the quadratic discriminant analysis MSPE is `r qda.miss`.
...
The test MSE for ridge and lasso regression were both approximately 0.42. It makes sense that these models would have worse test MSE values than other models since both ridge and lasso were unable to run with the non-numeric predictors. When we compare with the previous results, we see that some of the non-numeric (such as room-type and neighborhood group) were very important in predicting in other models.
...
* **Room Type**
  * Being a *Private Room* results in the price of the listing being `r (exp(-.7491)-1)*100`% lower than whole apartment or house listing.  
  * Being a *Shared Room* results in the price of the listing being `r (exp(-1.166)-1)*100`% lower than whole apartment or house listing.  
...
The logistic regression model ended up being the best choice for our classification model. As with the regression model, the misclassification error rate, `r final.logit.misclass`, was slightly higher than some of the more complex models, but we decided to choose the simpler model at a small penalty to misclassification rate. By searching for the best threshold, it seems setting $thresh > .5$ is the best choice. There seemed to be many more false positives than false negatives in our overall dataset predictions. As we can see from the confusion matrix of the logistic predictions, out False Postitive Rate was `r log.tab[2,1]/(log.tab[2,1] + log.tab[1,1])` and our False Negative Rate was `r log.tab[1,2]/(log.tab[2,2] + log.tab[1,2])`. 
...
One possible research direction for this dataset would be to use Host Name to classify each host as either male or female (or a combination of the two). One could then explore whether the host's gender played a role in predicting the amount of business the listing got or even combine that with the price to make inferences on the difference in net income from listings between genders. 
