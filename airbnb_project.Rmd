---
title: "Airbnb Listing Price Analysis"
...
The following project was created with the purpose of predicting prices of Airbnb rentals in New York City using various machine learning models. The dataset was downloaded from Kaggle, the original source being from insideairbnb.com, contains approximately 50,000 different Airbnb listings. Each listing had a variety of attributes including price, room type, latitude and longitude, along with information of the listing's host. Our models varied in complexity and accuracy, and no model was 100% correct, though many ended up being useful. As the old adage goes, there is no free lunch, and in the end, we concluded linear regression for log(price) prediction and logistic regression for price above median classification were the best models as it had the ideal combination of complexity, interpretability, and accuracy.  
...
* Ordinary Least Squares: Multiple Linear Regression with log(price) as the response variable. 
...
The MSPE for out ordinary least squares model was `r ols.mspe`. This is slightly higher than some of the methods we explore. Despite this, the model is simpler.
...
The ROC curve above shows how the true positive rate and false positive rate change based on the threshold we choose. The AUC for our logistic classifier is `r auc`, demonstrating our classifier is very good. Now, we can find the best threshold value for our logistic model:
...
Using the ROC curve we were able to find out the best threshold value to reduce the distance between the FPR and FNR. This threshold value was `r best.thresh`.

The confusion matrix outputted below gives an idea as to the distribution of predictions.

```{r final_train_logit, echo = F}
set.seed(123)
logit.fit <- glm(price_above ~longitude + latitude+ minimum_nights+ calculated_host_listings_count+ 
                    reviews_per_month + room_type + neighbourhood_group, data=train,
                 family=binomial('logit'))
...
#tr.tbl
lda.miss <- mean(yhat != test$price_above)
```
...
#tr.tbl
qda.miss <- 1-sum(diag(tr.tbl))/sum(tr.tbl)
```

The linear discriminant analysis misclassification rate is `r lda.miss` and the quadratic discriminant analysis MSPE is `r qda.miss`. These are very similar to the other classification methods we explore in our analysis.
...
The MSPE was the lowest for the model with `r best.tree` trees, so we implemented this into the final boosting model.
...
In order to gain a rudimentary understanding of our data, we plotted a scatterplot of the listings coded by price above or below the median. It appears that the price of properties is noticably higher along waterfront properties. 
...
```{r SVMpoly, echo=FALSE, warning=FALSE}
...
```{r KNN, echo = FALSE}
set.seed(3)
knn.caret <- train(as.factor(price_above) ~ neighbourhood_group+latitude+longitude+room_type+minimum_nights+number_of_reviews+calculated_host_listings_count+availability_365, data = train, method = "knn", trControl = trainControl("cv", number = 10), preProcess = c("center","scale"), tuneLength = 20)
plot(knn.caret)
```

```{r KNNMisclass, echo = FALSE}
set.seed(3)
...
### SVM and KNN Error Rates

```{r svm_knn_tab, echo = F}
...
### Regulatization Methods

```{r regul_table, echo = FALSE}
...
## Final Models and Overall Dataset Error
...
The final mean squared prediction error, `r final.ols.mspe`, of the model is slightly higher than some of the more complex models. This choice was made for our final model because the model is simpler and more efficient with larger datasets. While the accuracy is slightly decreased, it is not a large enough difference where we would consider another, more accurate model. Another reason for choosing the linear model is because of the ability to interpret the coefficients. Some of the more significant coefficients can be interpreted below:

*Note*: All interpretations are given all other predictors are held constant.

* **Latitude**: A one unit increase in latitude results in a `r (exp(-1.108)-1)*100`% decrease in the price of the listing.
* **Longitude**: A one unit increase in longitude results in a `r (exp(-2.569)-1)*100`% decrease in the price of the listing.
* **Neighbourhood Group**: 
  * Being in the *Brooklyn* neighbourhood group results in the price of the listing being `r (exp(-.07379)-1)*100`% lower than a listing in the Bronx.  
  * Being in the *Manhattan* neighbourhood group results in the price of the listing being `r (exp(.3154)-1)*100`% higher than a listing in the Bronx.  
  * Being in the *Queens* neighbourhood group results in the price of the listing being `r (exp(.05742)-1)*100`% higher than a listing in the Bronx.  
  * Being in the *Stanten Island* neighbourhood group results in the price of the listing being `r (exp(-.6991)-1)*100`% lower than a listing in the Bronx.  
* **Room Type**
  * Being a *Private Room* results in the price of the listing being `r (exp(-.7491)-1)*100`% lower than a *Whole Apartment or House* listing.  
  * Being a *Shared Room* results in the price of the listing being `r (exp(-1.166)-1)*100`% lower than a *Whole Apartment or House* listing.  
* **Calculated Host Listing Count**:  A one unit increase in longitude results in a `r (exp(.0005089)-1)*100`% decrease in the price of the listing.

### Classification

#### Logistic Regression

```{r final_logit, echo = F}
set.seed(123)
...
#kable(list(table = table(predicted = final.logit.pred,true = bnb$price_above)), caption = 'Confusion Matrix') %>% 
#  kable_styling(latex_options = "hold_position")

log.tab <- table(Predicted = final.logit.pred,True = bnb$price_above)
#kable(table(Predicted = final.logit.pred,True = bnb$price_above)) %>% kable_styling(latex_options = "hold_position")
log.tab
```

The logistic regression model ended up being the best choice for our classification model. As with the regression model, the misclassification error rate, `r final.logit.misclass`, was slightly higher than some of the more complex models, but we decided to choose the simpler model at a small penalty to misclassification rate. By searching for the best threshold, it seems `r best.thresh` is the best choice. There seemed to be many more false positives than false negatives in our overall dataset predictions. As we can see from the confusion matrix of the logistic predictions, our False Postitive Rate was `r log.tab[2,1]/(log.tab[2,1] + log.tab[1,1])` and our False Negative Rate was `r log.tab[1,2]/(log.tab[2,2] + log.tab[1,2])`. 
...
* **Neighbourhood Group**: 
  ...
  * Being in the *Stanten Island* neighbourhood group results in the log-odds of the price being below the median to be reduced by -3.371, compared to being a listing in the Bronx.
* **Room Type**
...
```{r ref.label="fpr", eval = F}

```

```{r ref.label="final_train_logit", eval = F}
...
```{r ref.label="SVMprep", eval = F}

```

```{r ref.label="SVMlinear", eval = F}

```

```{r ref.label="SVMpoly", eval = F}

```

```{r ref.label="SVMrad", eval = F}

```

```{r ref.label="KNN", eval = F}

```

```{r ref.label="KNNMisclass", eval = F}

```

```{r ref.label="ridge", eval = F}

```

```{r ref.label="lasso", eval = F}

```

```{r ref.label="linear_tab", eval = F}

```

```{r ref.label="tree_tab_final", eval = F}

```

```{r ref.label="svm_knn_tab", eval = F}

```

```{r ref.label="regul_table", eval = F}

```

```{r ref.label="final_ols", eval = F}

```

```{r ref.label="final_logit", eval = F}
